...
```{r}
datasets::penguins[c('species', 'island', 'bill_len')] |>
...
```{r}
datasets::penguins[c('sex', 'island', 'species', 'bill_dep')] |>
...
```{r}
tb = datasets::penguins[c('sex', 'island', 'species', 'bill_dep')] |>
  DemographicTable(by = ~ sex + island, data.name = 'datasets::penguins', compare = FALSE)
tb[-1L] |> 
  as_flextable()
```
...
A warning message will be printed if `logical` variables are used.

```{r}
tryCatch(DemographicTable(mtc, by = ~ am_manual), warning = identity)
```
